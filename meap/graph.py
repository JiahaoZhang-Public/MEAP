...
class Node:
    ...
    @property
    def neurons(self):
        if self.graph.neurons_in_graph is None:
            return None
        return self.graph.neurons_in_graph[self.graph.forward_index(self, attn_slice=False)]

    @neurons.setter
    def neurons(self, value):
        if self.graph.neurons_in_graph is None:
            raise RuntimeError(f"Cannot set score for node {self.name} because the graph does not have node scores enabled")
        self.graph.neurons_in_graph[self.graph.forward_index(self, attn_slice=False)] = value
