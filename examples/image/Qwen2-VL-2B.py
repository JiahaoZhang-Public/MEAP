...
Workflow:
1. Raw image + question pairs -> dataloader input
   (`PreparedBatch` or `RawPairBatch + pair_batch_preparer`)
2. Attribution (default: EAP)
3. Graph visualization export (JSON + PNG)

Run:
  python examples/image/Qwen2-VL-2B.py --device cpu --dtype float32 --method EAP --input-mode prepared
  python examples/image/Qwen2-VL-2B.py --device cpu --dtype float32 --method EAP --input-mode raw
"""
